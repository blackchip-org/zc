...
func main() {
	...
	// FIXME: Remove bootstrap when not necessary
	prelude := "user.prelude.zc"
	script, err := calc.Load(prelude)
	if err != nil {
		panic(err)
	}
	if err := calc.Eval(prelude, script); err != nil {
		panic(err)
	}
...
func evalLines(flags *flag.FlagSet, calc zc.Calc) {
	...
	for i, line := range flags.Args() {
		...
		if err = zc.EvalString(calc, name, line); err != nil {
			break
		}
	}
