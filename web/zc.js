...
function submit() {
    ...
    if (prev.length > 0) {
        for (let item of prev) {
            let text = annotate(item)
            output.push(`<li class='history'>${text}</li>`)
        }
        output.push(`<li class='history'>&nbsp;</li>`)
    }

    for (let i = 0; i < result.stack.length; i++) {
        let text = annotate(result.stack[i])
        let kind = (i === result.stack.length - 1) ? 'top-item' : 'stack-item'
        output.push(`<li class='${kind}'>${text}</li>`)
    }
    if (result.error !== '') {
...
function annotate(l) {
    let text = l.value
    if ( l.anno ) {
        text = `${text} <span class='anno'># ${l.anno}</span>`
    }
    return text
}
