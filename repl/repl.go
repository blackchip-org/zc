...
type REPL struct {
	...
	localDir    string
	historyFile string
	undoStack   []stack.Stack[zc.Item]
	redoStack   []stack.Stack[zc.Item]
...
func (r *REPL) Eval(line string) error {
	...
	if err == nil && cmdName != "undo" && cmdName != "u" && cmdName != "redo" {
		r.undoStack = append([]stack.Stack[zc.Item]{prev}, r.undoStack...)
		r.redoStack = nil
	}
	r.err = err
