...
func redo(r *REPL, _ *scan.Scanner) error {
	...
	r.undoStack = append([]stack.Stack[zc.Item]{r.Calc.Stack.Clone()}, r.undoStack...)
	r.Calc.Stack.SetItems(r.redoStack[0].Items())
...
func undo(r *REPL, _ *scan.Scanner) error {
	...
	r.redoStack = append([]stack.Stack[zc.Item]{r.Calc.Stack.Clone()}, r.redoStack...)
	r.Calc.Stack.SetItems(r.undoStack[0].Items())
