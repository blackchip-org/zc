...
func (r *Repl) Eval(line string) error {
	...
	if err == nil && cmdName != "undo" && cmdName != "u" && cmdName != "redo" {
		r.undoStack = append([][]zc.Item{prev}, r.undoStack...)
		r.redoStack = nil
	}
	r.err = err
	if r.Calc.Notice != "" && r.notice == "" {
		r.notice = r.Calc.Notice
	}
	return err
}
